...
export default function OfficerRegister() {
  ...
  const onSubmit = async (data: RegisterForm) => {
    try {
      setServerError('');
      await AuthService.registerOfficer(data);
      navigate('/officer', { replace: true });
    } catch (error: any) {
      setServerError(error.response?.data?.message || 'Failed to create officer account');
    }
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="space-y-2">
        <h2 className="text-3xl font-bold tracking-tight text-slate-900">Create Officer Account</h2>
        <p className="text-slate-500">Enter your details to create a new officer portal account.</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        >
          {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Create Officer Account'}
      ...
      <div className="text-center text-sm text-slate-500">
        ...
        <Link to="/officer/login" className="font-medium text-indigo-600 hover:text-indigo-500 transition-colors">
          Sign in here
        </Link>
      </div>
